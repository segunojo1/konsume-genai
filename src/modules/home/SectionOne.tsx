...
const SectionOne = () => {
  ...
  return (
    <section ref={containerRef} id="features" className="font-satoshi relative text-[40px]/[130%] md:text-[90px]/[120%] px-10 mt-56 font-bold z-[50] ">
      ...
      <motion.div
        ref={scrollRef}
        key="servup"
        className="flex md:flex-row flex-col items-center space-x-1 min-h-[150%] justify-center mt-12 mx-auto"
...
const FeatureCard = ({ top, rotate, cardimg, cardsticker, cardtitle, carddesc, colors, captionbg }: FeatureCardProps) => {
  return (
    <motion.figure  drag
    dragTransition={{ power: 0.1 }}   transition={{ ease: "easeOut", type: "spring", duration: 0.3, damping: 90 }} className={`z-[9999] relative p-6 overflow-hidden mt-[${top}px] md:min-h-[389px] min-h-[350px] bg-secondary-300 w-fit md:max-w-[290px] max-w-[260px] space-y-5 rounded-[21px] rotate-[${rotate}deg]`} style={{ transform: `rotate(${rotate}deg)`, backgroundColor: `${colors.bgcolor}`, color: colors.textcolor, marginTop: `${top}px` }}>
